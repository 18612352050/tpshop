...
//列表页点击全选按钮：修复此处的一个bug
$(function(){
	$('.list_tab').find('tr').find( 'th' ).find('[type=checkbox]').click(function(){
		$(this).parents( 'table' ).find( 'td' ).find('[type=checkbox]').prop('checked',$(this).prop('checked'));
	});

	$('.list_tab').find('tr').find( 'td' ).find('[type=checkbox]').click(function(){
		var middle = $('.list_tab').find('tr').find( 'th' ).find('[type=checkbox]').prop( 'checked' );
		if ( middle === true ) {
			$('.list_tab').find('tr').find( 'th' ).find('[type=checkbox]').prop( 'checked',false );
		}
	});
})
