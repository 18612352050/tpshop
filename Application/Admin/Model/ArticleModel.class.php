...
class ArticleModel extends Model{
	...
	protected function checkFilter(){
		...
		$post = $_POST;
		$post['art_thumb'] = ( isset( $post['art_thumb'] ) || !empty( $post['art_thumb'] ) ) ? $post['art_thumb'] : '/Public/default.png' ;
		$post['art_time'] = time();
		$post[ 'cate_pid' ] = M( 'Category' )->getFieldByCateId( $post[ 'cate_id' ],'cate_pid' );//栏目的父级
		//有主键的post数据为更新， 无主键的为新增，新增是不会进入此判断
		if( isset( $post[ 'art_id' ] ) ){
			$tmpstr = $this->getFieldByArtId( $post[ 'art_id' ],'art_thumb' );
			$tmp = pathinfo( $tmpstr );
			$postThumb = pathinfo( $post['art_thumb'] );
			//更新的封面图名字不等于默认图片名字，不等于旧的封面图
			if( $tmp['filename'] != 'newsdefault' && $tmp['filename'] != $postThumb['filename'] ) $thumbpath = $tmpstr;
		}
